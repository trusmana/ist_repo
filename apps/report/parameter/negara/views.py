...
class list_negara(View):
    context = {'segment': 'negara'}
    ...
    def list(self, request):
        ...
        pengiriman = Negara.objects.filter(filter_params) if filter_params else Negara.objects.all().order_by('-id')

        self.context['negara'], self.context['info'] = set_pagination(request, pengiriman)
        if not self.context['negara']:
            return False, self.context['info']

        return self.context, 'report/negara/data_negara.html'

    def edit(self, request, pk):
        pengiriman = self.get_object(pk)

        self.context['negara'] = pengiriman
        self.context['form'] = NegaraForm(instance=pengiriman)

        return self.context, 'report/negara/edit_negara.html'
    ...
    def edit_row(self, pk):
        ...
        return render_to_string('report/negara/edit_negara_row.html', context)

    """ Common methods """
        
    def get_object(self, pk):
        negara = get_object_or_404(Negara, id=pk)
        return negara
    
    def get_row_item(self, pk):
        negara = self.get_object(pk)
        edit_row = render_to_string('report/negara/edit_negara_row.html', {'instance': negara})
        return edit_row
    ...
    def update_instance(self, request, pk, is_urlencode=False):
        ...
        form = NegaraForm(form_data, instance=transaction)        
        if form.is_valid():
            form.save()
            if not is_urlencode:
