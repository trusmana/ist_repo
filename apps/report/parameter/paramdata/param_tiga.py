...
def addparam(request,pr,origin,through,destn):
    ...
    ### iD Gasti Asih , DPS Solid Dili Lintas Negara    
        ...
        if request.method == 'POST':
            ...
            form3 = fm.DLForm()
        return render(request,'report/parameter/add_parameter3.html',{'forms':form,'forms2':form2,'forms3':form3,'dt':dt})
    # NL SING DILI Wastila Fred Dili  
    elif origin == 12 and through == 1 and destn == 5:
        if request.method == 'POST':
            form = fm.WastilaForm(request.POST)
            form2 = fm.FSForm(request.POST)
            form3 = fm.DLForm(request.POST)
            if form.is_valid() and form2.is_valid() and form3.is_valid():
                saveform3(dt,form,form2,form3,user)
                messages.success(request, 'Data Parameter Berhasil Di Input')
                return redirect('d-param')
        else:
            form = fm.WastilaForm()
        ...
        return render(request,'report/parameter/add_parameter3.html',{'forms':form,'forms2':form2,'forms3':form3,'dt':dt})        
    ## FS DHL DILI
    elif origin == 1 and through == 8 and destn == 5:
        if request.method == 'POST':
            form = fm.FSForm(request.POST)
            form2 = fm.DHLForm(request.POST)
            form3 = fm.DLForm(request.POST)
            if form.is_valid() and form2.is_valid() and form3.is_valid():
                saveform4(dt,form,form2,form3,user)
                messages.success(request, 'Data Parameter Berhasil Di Input')
                return redirect('d-param')
        else:
            form = fm.FSForm()
...
####Khusus FS DHL DILI
def saveform4(dt,form,form2,form3,user):
    ###FS
    ...
    ###fs
    ### DHL 
    min_express_wordwide_nondoc = form2.cleaned_data['min_express_wordwide_nondoc']
    price_express_wordwide_nondoc = form2.cleaned_data['price_express_wordwide_nondoc']
    price_high_express_wordwide_nondoc = form2.cleaned_data['price_high_express_wordwide_nondoc']
    
    min_fuel_surcharge_dhl = form2.cleaned_data['min_fuel_surcharge_dhl']
    price_fuel_surcharge_dhl = form2.cleaned_data['price_fuel_surcharge_dhl']
    price_high_fuel_surcharge_dhl = form2.cleaned_data['price_high_fuel_surcharge_dhl']

    min_emergency_situation = form2.cleaned_data['min_emergency_situation']
    price_emergency_situation = form2.cleaned_data['price_emergency_situation']
    price_high_emergency_situation = form2.cleaned_data['price_high_emergency_situation']    
    ...
    prm = ParameterData(products = dt,j_vendor=3,status_param = 1,tgl_aktif_param= datetime.date.today(),cu=user,
        ...
        price_gst_zero_rated = price_gst_zero_rated,###fs
        ##dl
...
####Khusus WARSILA FS DL
def saveform3(dt,form,form2,form3,user):
    ###WS
    min_custom_learance_fee_handling= form.cleaned_data['min_custom_learance_fee_handling']
    price_custom_learance_fee_handling= form.cleaned_data['price_custom_learance_fee_handling']
    price_high_custom_learance_fee_handling= form.cleaned_data['price_high_custom_learance_fee_handling']
    min_heavy_weight_surcharge= form.cleaned_data['min_heavy_weight_surcharge']
    price_heavy_weight_surcharge= form.cleaned_data['price_heavy_weight_surcharge']
    price_high_heavy_weight_surcharge= form.cleaned_data['price_high_heavy_weight_surcharge']    
    min_agent_fee= form.cleaned_data['min_agent_fee']
    price_agent_fee= form.cleaned_data['price_agent_fee']
    price_high_agent_fee= form.cleaned_data['price_high_agent_fee']
    min_delivery = form.cleaned_data['min_delivery']
    price_delivery= form.cleaned_data['price_delivery']
    price_high_delivery= form.cleaned_data['price_high_delivery']
    ###WS
    ###FS
    min_airfreight = form2.cleaned_data['min_airfreight']
    price_min_airfreight = form2.cleaned_data['price_min_airfreight']
    price_max_airfreight = form2.cleaned_data['price_max_airfreight']
    price_max_handling_charges = form2.cleaned_data['price_max_handling_charges']
    min_handling_charges = form2.cleaned_data['min_handling_charges']
    price_min_handling_charges = form2.cleaned_data['price_min_handling_charges']
    
    min_insurance_security_surcharge = form2.cleaned_data['min_insurance_security_surcharge']
    price_insurance_security_surcharge = form2.cleaned_data['price_insurance_security_surcharge']
    price_high_insurance_security_surcharge = form2.cleaned_data['price_high_insurance_security_surcharge']
    
    min_fuel_surcharge = form2.cleaned_data['min_fuel_surcharge']
    price_fuel_surcharge = form2.cleaned_data['price_fuel_surcharge']
    price_high_fuel_surcharge = form2.cleaned_data['price_high_fuel_surcharge']
    
    min_import_handling_charges = form2.cleaned_data['min_import_handling_charges']
    price_import_handling_charges = form2.cleaned_data['price_import_handling_charges']
    price_high_import_handling_charges = form2.cleaned_data['price_high_import_handling_charges']
    min_gst_zero_rated = form2.cleaned_data['min_gst_zero_rated']
    price_gst_zero_rated = form2.cleaned_data['price_gst_zero_rated']
    price_high_gst_zero_rated = form2.cleaned_data['price_high_gst_zero_rated']
    ###fs    
    ...
    prm = ParameterData(products = dt,j_vendor=3,status_param = 1,tgl_aktif_param= datetime.date.today(),cu=user,
        ## WS
        min_custom_learance_fee_handling=min_custom_learance_fee_handling,price_custom_learance_fee_handling=price_custom_learance_fee_handling,
        price_high_custom_learance_fee_handling =price_high_custom_learance_fee_handling,
        min_heavy_weight_surcharge = min_heavy_weight_surcharge,price_heavy_weight_surcharge =price_heavy_weight_surcharge,
        price_high_heavy_weight_surcharge= price_high_heavy_weight_surcharge,    
        min_agent_fee= min_agent_fee,price_agent_fee= price_agent_fee,price_high_agent_fee= price_high_agent_fee,
        min_delivery = min_delivery,price_delivery= price_delivery,price_high_delivery= price_high_delivery,
        ###fs
        min_airfreight = min_airfreight,price_min_airfreight = price_min_airfreight,price_max_airfreight = price_max_airfreight,
        price_max_handling_charges = price_max_handling_charges,min_handling_charges = min_handling_charges,
        price_min_handling_charges = price_min_handling_charges,
