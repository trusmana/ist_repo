...
class list_param(View):
    ...
    def list(self, request):
        ...
        param = ParameterData.objects.filter(filter_params) if filter_params else ParameterData.objects.all().order_by('-id')
    ...
    def edit(self, request, pk):
        ...
        self.context['form'] = ParamAllForm(instance=param)
...
def addparam(request):
    ...
    if request.method == 'POST':
        form = ParamAllForm(request.POST)
        if form.is_valid():
        ...
        form = ParamAllForm()
    return render(request,'report/parameter/add_parameter.html',{'form':form})
