...
class GastiAsih(models.Model):
    ...
    rate = models.DecimalField(max_digits=12, decimal_places=2,null=True,blank=True,default=0)

    class Meta:
        db_table = 'gastiasih'

    def __str__(self):
        return '%s %s %s %s %s'%(self.id, self.name_vendor,self.jenis_angkutan,self.origin, self.destinations)
