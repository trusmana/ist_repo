...
class list_hsc(View):
    ...
    def list(self, request):
        ...
        transactions = mp.Invoice.objects.filter(filter_params) if filter_params else mp.Invoice.objects.all().order_by('id')
        ...
        return self.context, 'menu_invoice/gabung/invoice_gabung.html'

    def edit(self, request, pk):
        transaction = self.get_object(pk)

        self.context['transaction'] = transaction # type: ignore
        self.context['form'] = fv.HscForm(instance=transaction)# type: ignore

        return self.context, 'menu_invoice/gabung/edit.html'

    """ Get Ajax pages """

    def edit_row(self, pk):
        transaction = self.get_object(pk)
        form = fv.HscForm(instance=transaction)
        context = {'instance': transaction, 'form': form}
        return render_to_string('menu_invoice/gabung/edit_row.html', context)

    """ Common methods """
        
    def get_object(self, pk):
        transaction = get_object_or_404(mp.Invoice, id=pk)
        return transaction
    
    def get_row_item(self, pk):
        transaction = self.get_object(pk)
        edit_row = render_to_string('menu_invoice/gabung/edit_row.html', {'instance': transaction})
        return edit_row

    def update_instance(self, request, pk, is_urlencode=False):
        transaction = self.get_object(pk)
        form_data = QueryDict(request.body) if is_urlencode else request.POST
        form = fv.HscForm(form_data, instance=transaction)
        if form.is_valid():
            form.save()
            if not is_urlencode:
...
def addhsc(request):
    ...
    if request.method == 'POST':
        ...
        form = fv.HscForm(initial={'status':1})
    return render(request,'menu_invoice/gabung/add_alt_gabung.html',{'form':form})